...
#### 4）(未完成)  ActivityManagerService.setSystemProcess()分析
...
![AMS Pause](/Users/shenyutao/Desktop/AMS Pause.jpg)
...
该方法先将DecorView通过`getWindowManager()`获取到的ViewManager（这里就是指WindowManagerImpl）加入WMS中，然后显示DecorView
...
root.setView 会调用 requestlayout() 这个之前分析过，它会开启View的三大流程，最后DecorView就被显示出来了
